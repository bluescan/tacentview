...
// format and loads the data into tLayers. This class does not compress or decompress the image data if it is
// compressed (DXTn), it simply keeps it in the same format as the source file. The layers may be 'stolen' from a
// tFileDDS so that excessive memcpys are avoided. After they are stolen the tFileDDS is invalid.
//
...
// A tFileDDS object represents and knows how to load a dds file. In general a DirectDrawSurface is composed of
// multiple layers -- each one a mipmap. This class does not compress or decompress the image data if it is compressed
// (DXTn), it simply keeps it in the same format as the source file. The layers may be 'stolen' from a tFileDDS so that
// excessive memcpys are avoided.
class tFileDDS
{
public:
	// Creates an invalid tFileDDS. You must call Load manually.
	tFileDDS();
	...
	// this class does.
	tFileDDS(const tString& ddsFile, bool correctRowOrder = true);
	...
	tFileDDS(const uint8* ddsFileInMemory, int numBytes, bool correctRowOrder = true);
	virtual ~tFileDDS()																									{ Clear(); }

	// Clears the current tFileDDS before loading. If the dds file failed to load for any reason it will throw a
	...
	// the tFileDDS is a cubemap, this function returns false and leaves the object unmodified.
	...
	// Similar to StealTextureLayers except it steals up to 6 layer-lists if the object is a cubemap. If the tFileDDS
...
// Error objects of this type may be thrown by the tFileDDS class if something went wrong.
