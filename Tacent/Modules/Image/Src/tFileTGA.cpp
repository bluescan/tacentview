...
bool tFileTGA::Load(const tString& tgaFile)
{
	Clear();
...
bool tFileTGA::Set(const uint8* tgaFileInMemory, int numBytes)
{
	Clear();
...
void tFileTGA::ReadColourBytes(tColouri& dest, const uint8* src, int bytesPerPixel)
{
...
bool tFileTGA::Set(tPixel* pixels, int width, int height, bool steal)
{
	Clear();
...
tFileTGA::tFormat tFileTGA::Save(const tString& tgaFile, tFormat format, tCompression compression) const
{
...
bool tFileTGA::SaveUncompressed(const tString& tgaFile, tFormat format) const
{
...
bool tFileTGA::SaveCompressed(const tString& tgaFile, tFormat format) const
{
	if ((format != tFormat::Bit24) && (format != tFormat::Bit32))
		return false;
