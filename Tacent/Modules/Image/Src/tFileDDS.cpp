...
tFileDDS::tFileDDS(const tString& ddsFile, bool reverseRowOrder) :
	Filename(ddsFile),
	PixelFormat(tPixelFormat::Invalid),
	IsCubeMap(false),
	NumImages(0),
...
tFileDDS::tFileDDS(const uint8* ddsFileInMemory, int numBytes, bool reverseRowOrder) :
	Filename(),
	PixelFormat(tPixelFormat::Invalid),
	IsCubeMap(false),
	NumImages(0),
...
bool tFileDDS::StealTextureLayers(tList<tLayer>& layers)
{
...
int tFileDDS::StealCubemapLayers(tList<tLayer> layerLists[tSurfIndex_NumSurfaces], uint32 sideFlags)
{
...
// These DXT blocks are needed so that the tFileDDS class can re-order the rows by messing with each block's lookup
...
void tFileDDS::Load(const tString& ddsFile, bool reverseRowOrder)
{
	Clear();
...
void tFileDDS::Load(const uint8* ddsFileInMemory, int ddsSizeBytes, bool reverseRowOrder)
{
	Clear();
...
void tFileDDS::LoadFromMemory(const uint8* ddsData, int ddsSizeBytes, bool reverseRowOrder)
{
...
bool tFileDDS::DoDXT1BlocksHaveBinaryAlpha(tDXT1Block* block, int numBlocks)
{
