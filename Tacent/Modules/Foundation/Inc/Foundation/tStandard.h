...
// For these conversion calls, unknown digit characters for the supplied base are ignored. If base is not E [2, 36],
// the base in which to interpret the string is determined by passing a prefix in the string. Base 10 is used if no
// specific prefix is found. Base prefixes in use:
//
...
// include + and - for non base 10. Finally the conversion is performed. Valid digits for base 36 are 0-9, a-z, and
// A-Z. In all string-to-int functions, 0 is returned if there is no conversion. This can happen if all characters
// are invalid, the passed in string is null, or the passed in string is empty.
//
// If base is E (1, 36] AND a prefix is supplied, the supplied base is used. Using the prefix instead would be
// ambiguous. For example, "0x" is a valid base 36 number but "0x" is also a prefix. If you supply a prefix it
// must match the base or you will get undefined results. eg "0xff" or "xff" with base=36 interprets the 'x',
// correctly, as 33. "0xff" with base=16 works too because 'x' is an invalid hex digit and gets ignored. The '0' is
// leading so also does not interfere. The same behaviour holds for all prefixes and bases. Finally, don't pass
// a base-34 number in with the base set to -1, although that shouldn't happen anyways.
...
// These are both base 10 only. They return 0.0f if there is no conversion.s
...
inline double tStrtod(const char*);
