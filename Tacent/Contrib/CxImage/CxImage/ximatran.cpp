	...
	for (int32_t y=0;y<head.biHeight;y++){
		...
		for (int32_t x=0;x<head.biWidth;x++){
			if (!errordiffusion){
				...
				c.rgbRed = (uint8_t)cxmin(255L,cxmax(0L,(int32_t)c.rgbRed + ((er*7)/16)));
				c.rgbGreen = (uint8_t)cxmin(255L,cxmax(0L,(int32_t)c.rgbGreen + ((eg*7)/16)));
				c.rgbBlue = (uint8_t)cxmin(255L,cxmax(0L,(int32_t)c.rgbBlue + ((eb*7)/16)));
				...
				for(int32_t i=-1; i<2; i++){
					...
					c.rgbRed = (uint8_t)cxmin(255L,cxmax(0L,(int32_t)c.rgbRed + ((er * coeff)/16)));
					c.rgbGreen = (uint8_t)cxmin(255L,cxmax(0L,(int32_t)c.rgbGreen + ((eg * coeff)/16)));
					c.rgbBlue = (uint8_t)cxmin(255L,cxmax(0L,(int32_t)c.rgbBlue + ((eb * coeff)/16)));
...
bool CxImage::Dither(int32_t method)
{
	...
	case 2:
	{
		...
		for (int32_t y = 0; y < head.biHeight; y++) {
			...
			for (int32_t x = 0; x < head.biWidth; x++) {
				...
				level = (uint8_t)cxmin(255, cxmax(0, (int32_t)nlevel));
	...
	case 7:
	{
		...
		int32_t scale = cxmax(0,(8-2*order));
		int32_t level;
		for (int32_t y=0;y<head.biHeight;y++){
			info.nProgress = (int32_t)(100*y/head.biHeight);
			if (info.nEscape) break;
...
bool CxImage::CropRotatedRectangle( int32_t topx, int32_t topy, int32_t width, int32_t height, float angle, CxImage* iDst)
{
	...
	startx = cxmin(topx, topx - (int32_t)(sin_angle*(double)height));
...
bool CxImage::Crop(int32_t left, int32_t top, int32_t right, int32_t bottom, CxImage* iDst)
{
	...
	int32_t startx = cxmax(0L,cxmin(left,head.biWidth));
	int32_t endx = cxmax(0L,cxmin(right,head.biWidth));
	int32_t starty = head.biHeight - cxmax(0L,cxmin(top,head.biHeight));
	int32_t endy = head.biHeight - cxmax(0L,cxmin(bottom,head.biHeight));
